...
class ToolResult:
    ...
    Status values:
    - "success": Tool executed successfully with meaningful results
    - "no_results": Tool executed but found nothing useful (not an error)
    - "error": Tool execution failed
    
    The 'message' field provides a human-readable string that the LM
    can use directly to formulate its response. For "success" status,
    this contains the tool output. For "no_results" and "error", it
    contains a brief explanation.
    
    Attributes:
        status: "success", "no_results", or "error"
        message: Human-readable result text (for LM to use in responses)
        data: Structured data payload (only when status="success")
        error: Error details (only when status="error")
        success: Deprecated alias for status="success". Kept for backward compat.
        content: Deprecated alias for message. Kept for backward compat.
    """
    status: str = "success"
    message: str = ""
    data: str = ""
