...
class JsonAwareFormatter(logging.Formatter):
    """
    Custom logging formatter that handles long messages intelligently.
    
    - Short messages: formatted normally on a single line
    - Long JSON messages: pretty-printed with indentation across multiple lines
    - Long non-JSON messages: wrapped at max_line_length
    
    This prevents extremely long log lines that break line-by-line log readers.
    """
    
    def __init__(self, fmt=None, datefmt=None, max_line_length: int = 200):
        """
        Initialize the formatter.
        
        Args:
            fmt: Format string for the log record
            datefmt: Format string for timestamps
            max_line_length: Maximum line length before triggering wrapping
        """
        super().__init__(fmt, datefmt)
        self.max_line_length = max_line_length
    
    def _try_parse_json(self, message: str) -> Optional[object]:
        """Try to parse a message as JSON. Returns parsed object or None."""
    ...
    def _format_json_message(self, header: str, parsed: object) -> str:
        """Format a JSON message with pretty printing."""
        pretty_json = json.dumps(parsed, indent=2, ensure_ascii=False, default=str)
        return f"{header}\n{pretty_json}"
    
    def _wrap_text(self, text: str, max_length: int, indent: str = "") -> str:
        """Wrap text to max_length, preserving word boundaries."""
    ...
    def format(self, record: logging.LogRecord) -> str:
        """
        Format a log record, handling long JSON messages specially.
        ...
        # Short messages: format normally
        if len(message) <= self.max_line_length:
            return super().format(record)
        
        # Ensure asctime and message are computed before formatMessage()
        # formatMessage() calls _style.format(record) which needs these attributes
        record.asctime = self.formatTime(record, self.datefmt)
        record.message = message
        
        # Long message: try to parse as JSON
        parsed = self._try_parse_json(message)
        if parsed is not None:
            # Format header (timestamp, level, module)
            header = self.formatMessage(record)
            return self._format_json_message(header, parsed)
        
        # Not JSON: wrap the message
        header = self.formatMessage(record)
        wrapped_body = self._wrap_text(message, self.max_line_length, indent=" " * 4)
        return f"{header}\n{wrapped_body}"
...
class _TeeStream:
    ...
    def write(self, text: str):
        """Write to both the log file and original stream, splitting excessively long lines."""
        # Split lines that exceed MAX_LINE_LENGTH into multiple lines for the log file
        if self._file:
            try:
                wrapped = self._split_long_lines(text)
                self._file.write(wrapped)
                self._file.flush()
            except (OSError, ValueError):
                pass
        # Write to original stream (no wrapping for terminal display)
        try:
    ...
    def _split_long_lines(self, text: str, max_line_length: int = 200) -> str:
        """
        Split lines in text that exceed max_line_length into multiple lines.
        
        Instead of truncating, long lines are word-wrapped to spread across
        multiple lines, preventing line-by-line log readers from crashing.
        
        Args:
            text: The text to process
            max_line_length: Maximum allowed line length (default: 200)
            
        Returns:
            Text with long lines split into multiple lines
        """
        if not text:
            return text
        ...
        for line in lines:
            if len(line) <= max_line_length:
                ...
                # Word-wrap the long line into multiple lines
                words = line.split()
                current_line = ""
                for word in words:
                    if not current_line:
...
class Logger:
    ...
    def _format_message_for_file(self, entry: LogEntry) -> str:
        """
        Format a log entry's message for file output, handling long JSON messages.
        
        Args:
            entry: The log entry to format
            
        Returns:
            Formatted log line(s)
        """
        message = entry.message
        timestamp = entry.timestamp.strftime('%Y-%m-%d %H:%M:%S')
        level = entry.level.name
        module = entry.module or 'App'
        ...
        parsed = self._try_parse_json(message)
        if parsed is not None:
            header = f"[{timestamp}] [{level}] [{module}]"
            pretty_json = json.dumps(parsed, indent=2, ensure_ascii=False, default=str)
            return f"{header}\n{pretty_json}\n"
        
        # Not JSON: wrap at 200 characters
        header = f"[{timestamp}] [{level}] [{module}]"
        lines = []
        words = message.split()
        current_line = "    "  # 4-space indent for message body
        
        for word in words:
            if len(current_line) + len(word) + 1 > 204:  # 200 + header length
                lines.append(current_line.rstrip())
                current_line = "    "
            current_line += word + " "
    ...
    @staticmethod
    def _try_parse_json(message: str) -> Optional[object]:
    ...
    def _write_to_file(self, entry: LogEntry):
        ...
        if not self.log_to_file:
            return
        try:
            formatted = self._format_message_for_file(entry)
...
def setup_logging(module_filter: Optional[Dict[str, str]] = None) -> LoggingHandler:
    ...
    1. Python's standard logging with JsonAwareFormatter — handles long JSON messages
    ...
    Terminal output:
    - JsonAwareFormatter detects long JSON messages and pretty-prints them
    - Non-JSON long messages are word-wrapped at 200 characters
    ...
    # Configure Python's standard logging with JsonAwareFormatter
    # This formatter detects long JSON messages and pretty-prints them
    formatter = JsonAwareFormatter(
    ...
    # Apply the custom formatter to all existing handlers
