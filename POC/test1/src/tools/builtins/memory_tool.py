        ...
        try:
            if operation == "save":
                ...
                return ToolResult(
                    success=False,
                    content="",
                    error=f"Unknown operation: {operation}. Valid operations: save, search, retrieve, list, delete, statistics, search_recent, search_by_importance"
                )
    ...
    def _search_memories(self, kwargs: dict) -> ToolResult:
        ...
        if not keywords:
            return ToolResult(
                success=False,
                content="",
                error="Missing required parameter: keywords (list of search terms)"
            )
